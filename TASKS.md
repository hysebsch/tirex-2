...
- **Handoff (2026-08-03):** Work is on branch `main` of fork `hysebsch/tirex-2` (latest commit `a12c3f3`), upstream `NX-AI/tirex-2`. Next choices: (1) implement `TimeSeriesClassifier`, (2) implement `HardwareOptimizer.compile/quantize`, (3) open PR to upstream, (4) validate `make install-cuda` on DGX Spark.
